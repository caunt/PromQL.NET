...
namespace PromQL.Vectors
{
    public partial class RangeVector : AbstractVector<RangeVector>
    {
        ...
        public static implicit operator string(RangeVector vector)
        {
            ...
            {
                var unit when unit == TimeUnit.Milliseconds => vector.range.TotalMilliseconds,
                var unit when unit == TimeUnit.Seconds => vector.range.TotalSeconds,
                var unit when unit == TimeUnit.Minutes => vector.range.TotalMinutes,
                var unit when unit == TimeUnit.Hours => vector.range.TotalHours,
                var unit when unit == TimeUnit.Days => vector.range.TotalHours / 24, // assuming a day has always 24h
                var unit when unit == TimeUnit.Weeks => vector.range.TotalHours / 24 / 7, // assuming a week has always 7d
                var unit when unit == TimeUnit.Years => vector.range.TotalHours / 24 / 7 / 365, // assuming a year has always 365d
